...
export default function EditCareerPage() {
  ...
  useEffect(() => {
    const fetchCareer = async () => {
      try {
        const data = await careersApi.getById(params.id as string);
        setCareer(data);
        setForm({ title: data.title, location: data.location, employmentType: data.employmentType, description: data.description, status: data.status });
      } catch {
        toast({ title: 'Error', description: 'Failed to load job opening', variant: 'destructive' });
        router.push('/careers');
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await careersApi.update(params.id as string, form);
      toast({ title: 'Success', description: 'Job opening updated successfully' });
      router.push('/careers');
    } catch {
      toast({ title: 'Error', description: 'Failed to update job opening', variant: 'destructive' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-3xl font-bold">Edit Job Opening</h1><p className="text-muted-foreground">{career?.title}</p></div>
      </div>
      <Card>
